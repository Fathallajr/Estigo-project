...
export class AuthService {
  ...
  login(email: string, password: string) {
    return this.http.post<LoginResponse>("https://estigo.runasp.net/api/Auth/login", { email, password })
      .pipe(
        tap(response => {
          if (this.isBrowser) {
  ...
  registerTeacher(data: RegisterResponseTeacher) {
    ...
    return this.http.post<LoginResponse>("https://estigo.runasp.net/api/Auth/register/teacher", formData);
  }

  registerStudent(data: RegisterResponseStudent) {
    const headers = new HttpHeaders().set('Content-Type', 'application/json');
    return this.http.post<LoginResponse>("https://estigo.runasp.net/api/Auth/register/student", data, { headers });
  }

  registerParent(data: RegisterResponseParent) {
    const headers = new HttpHeaders().set('Content-Type', 'application/json');
    return this.http.post<LoginResponse>("https://estigo.runasp.net/api/Auth/register/parent", data, { headers });
  }
} 